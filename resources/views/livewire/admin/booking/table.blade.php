                ...
                <div class="card-body">
                    ...
                    <div class="card-body  search-group">
                        <div class="row">
                            ...
                            <div class="col-md-3 ">
                                <select wire:model="search_specialty_id" class="form-control"  value="{{ $search_specialty_id ?? '' }}" id="">
                                    <option value=""> @lang('specialties.specialties') </option>
                                    @forelse($specialties as $key => $specialty)
                                        <option value="{{ $specialty->id }}"> {{ $specialty->trans->where('locale',$current_lang)->first()->title }}</option>
                                    @empty
                                    @endforelse
                                </select>
                            </div>
                            <div class="col-md-3 ">
                                <select wire:model="search_doctor_id" class="form-control"  value="{{ $search_doctor_id ?? '' }}" id="">
                                    <option value=""> @lang('doctors.doctors') </option>
                                    @forelse($doctors as $key => $doctor)
                                        <option value="{{ $doctor->id }}"> {{ $doctor->trans->where('locale',$current_lang)->first()->title }}</option>
                                    @empty
                                    @endforelse
                                </select>
                            </div>
                    ...
                    <table class="table table-bordered dt-responsive nowrap"
                        ...
                        <thead>
                            ...
                            <tr>
                                ...
                                <th>#</th>
                                <th>{{ trans('specialties.specialties') }}</th>
                        ...
                        <tbody>
                            ...
                            @forelse ($items as $key => $item)
                                ...
                                @include('livewire.admin.booking.show_message_modal')
