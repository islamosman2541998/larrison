...
@section('title', trans('articles.show_articles'))
@section('title_page', trans('articles.show_articles'))
                        ...
                        <form action="{{ route('admin.articles.index') }}" method="get">
                            <div class="row mb-3">
                                ...
                                <div class="search-input col-md-2">
                                    ...
                                    <a class="btn btn-success btn-sm" href="{{ route('admin.articles.index') }}"><i
                                ...
                                <tbody>
                                    ...
                                    @foreach ($items as $key => $item)
                                        ...
                                        @foreach ($item->tags as $tag)
                                         <span class="badge bg-success">{{$tag->trans->where('locale',$current_lang)->first()->title }}</span>
                                        @endforeach
                                    </td>
                                    <td>{{ @ $item->categories ? $item->categories->trans->where('locale',$current_lang)->first()->title : ""}}</td> --}}
                                            <td>{{ $item->created_at }}</td>
                                            <td>{{ $item->updated_at }}</td>
                                            <td>
                                                <div class="d-flex justify-content-center">
                                                    @if ($item->status == 1)
                                                        <a href="{{ route('admin.articles.update-status', $item->id) }}"
                                                    ...
                                                    @if ($item->feature == 1)
                                                        <a href="{{ route('admin.articles.update-featured', $item->id) }}"
                                        ...
                                        @include('admin.dashboard.articles.delete')
                                    @endforeach
