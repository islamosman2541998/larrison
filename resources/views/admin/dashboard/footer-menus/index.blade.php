...
@if (\Route::current()->getName() == 'admin.footer-menus.edit')
    @section('title', trans('footerMenu.show_menus'))
    @section('title_page', trans('pages.edit', ['name' => @$menu->title]) )
@elseif (\Route::current()->getName() == 'admin.footer-menus.create')
    @section('title', trans('footerMenu.create_menus'))
    @section('title_page', trans('footerMenu.create_menus'))
@else 
    @section('title', trans('footerMenu.show_menus'))
    @section('title_page', trans('footerMenu.show_menus'))
@endif
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="actions-tree mb-3">
                            <form action="{{ route('admin.footer-menus.show_tree') }}" method="GET">
                            ...
                            <a href="{{ route('admin.footer-menus.create') }}" title="@lang('admin.create')" class="btn btn-success" > @lang('admin.create')</i></a>
                        </div>
                        @php 
                        ...
                        <ul class="todos" id="todos">
                            @include('admin/dashboard/footer-menus/tree')

                            {{-- <li class="todo-plus">
                                <a href="{{ route('admin.footer-menus.create', 0) }}" title="@lang('admin.create')" class="text-success" ><i class="fas fa-plus"></i></a>
                            </li> --}}
                        </ul>
                    </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            @if( @$createMode != true && (@$menu->children == null ||  @$menu->children ->first() == null) )
                                                <div class="destroy text-end">
                                                    <a class="btn btn-danger btn-sm m-1" data-bs-toggle="modal" data-bs-target="#exampleModal{{ @$menu->id }}">
                                                        @lang('admin.delete')
                                                    </a>
                                                    <div class="modal fade" id="exampleModal{{ @$menu->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                <form  action="{{ route('admin.footer-menus.destroy' , @$menu->id) }}" method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
                                            ...
                                            @if(@$createMode == true)
                                                ...
                                                <form action="{{ route('admin.footer-menus.store') }}" method="post">
                                                    <input type="hidden" name="parent_id" value="{{ @$item_parent_id == 0? NULL:  @$item_parent_id }}">
                                            @else
                                                <form action="{{ route('admin.footer-menus.update', $menu->id) }}" method="post">
                                            ...
                                            @endif
                                                ...
                                                <div class="row">
                                                                        ...
                                                                        <div class="accordion-body">
                                                                            ...
                                                                            {{-- title ------------------------------------------------------------------------------------- --}}
                                                                                <div class="row mb-3">
                                                                                    ...
                                                                                    <div class="col-sm-10">
                                                                                        <input type="text" id="title{{ $key }}" name="{{ $locale }}[title]" value="{{ @$menu != null ? optional( $menu->trans)->where('locale', $locale)->first()->title : old($locale . '.title') }}" class="form-control title" required>
                                                                                    </div>
                                                                            ...
                                                                            {{-- slug ------------------------------------------------------------------------------------- --}}
                                                                                <div class="row mb-3 slug-section">
                                                                                    ...
                                                                                    <div class="col-sm-10">
                                                                                        <input type="text" id="slug{{ $key }}" name="{{ $locale }}[slug]" value="{{ @$menu != null ? optional( $menu->trans)->where('locale', $locale)->first()->slug : old($locale . '.slug') }}" class="form-control slug" required>
                                                                                    </div>
                                                    ...
                                                    <div class="col-md-12">
                                                        <div class="accordion mt-4 mb-4" id="accordionExample">
                                                            ...
                                                            <h2 class="accordion-header" id="headingOne">
                                                                <button class="accordion-button fw-medium" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOn" aria-expanded="true" aria-controls="collapseOne{{ $key }}">
                                                                    {{ trans('footerMenu.Settings') }}
                                                                </button>
                                                            </h2>
                                                            <div id="collapseOne" class="accordion-collapse collapse show mt-3" aria-labelledby="headingOn" data-bs-parent="#accordionExample">
                                                                <div class="accordion-body">
                                                                    
                                                                    <div class="row mb-3 title-section">
                                                                        <label for="example-text-input"  class="col-sm-2 col-form-label">{{  trans('footerMenu.type')}}</label>
                                                                        <div class="col-sm-10">
                                                                            <select class="form-select form-select-sm" name="type" id="type" aria-label=".form-select-sm example">
                                                                                <option selected>@lang('footerMenu.type') </option>
                                                                    ...
                                                                    <div class="row mb-3 title-section" id="static-url"  @if( @$menu->type != App\Enums\MunesEnums::STATIC) style="display: none;"  @endif>
                                                                        <label for="example-text-input"  class="col-sm-2 col-form-label">{{  trans('footerMenu.url') }}</label>
                                                                        <div class="col-sm-10">
                                                                    ...
                                                                    <div id="dynamic-url" @if( @$menu->type != App\Enums\MunesEnums::DYNAMIC) style="display: none;"  @endif>
                                                                        <div class="row mb-3 title-section"  >
                                                                            <label for="example-text-input"  class="col-sm-2 col-form-label">{{  trans('footerMenu.type_url') }}</label>
                                                                            <div class="col-sm-10">
                                                                                <select class="form-select form-select-sm" id="dynamic-type" name="dynamic_table" aria-label=".form-select-sm example">
                                                                                    ...
                                                                                    @foreach(App\Enums\UrlTypesEnum::values() as $enumTypes)
                                                                                        <option value="{{ $enumTypes }}" {{  $enumTypes == @$menu->dynamic_table ? "selected":"" }}>{{ $enumTypes }}</option>
                                                                                    @endforeach
                                                                                </select>
                                                                                @if('type')
                                                                    ...
                                                                    <div class="row mb-3 title-section">
                                                                        <label for="example-text-input"  class="col-sm-2 col-form-label">{{  trans('footerMenu.status') }}</label>
                                                                        <div class="col-sm-10">
                                                                            <input class="form-check form-switch" name="status" type="checkbox" id="switch4" switch="success"  {{ @$menu->status == 1 || old('url') ==1 ?'checked':'' }} value="1">
                                                    ...
                                                    <div class="row mb-3 text-end">
                                                        <div>
                                                            <a href="{{ route('admin.footer-menus.index') }}"
