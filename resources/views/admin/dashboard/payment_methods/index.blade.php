...
@section('title_page', trans('admin.payment_methods'))
...
{{--                        <a href="{{ route('admin.payment-methods.create') }}" class="btn btn-outline-success btn-sm">@lang('admin.create')</a>--}}
{{--                    </div>--}}
                </div>
{{--                <form action="{{route('admin.payment-methods.index')}}" method="get">--}}
{{--                    <div class="row">--}}
{{--                        <div class="col-md-3 mb-2">--}}
{{--                            <input type="test" value="{{ request()->title != '' ? request()->title : ''}}" name="title" placeholder="{{ trans('pages.search_title') }}" class="form-control">--}}
{{--                        </div>--}}
...
{{--                            <button class="btn btn-primary btn-sm" type="submit" title="{{ trans('button.search') }}"><i class="fas fa-search"> </i></button>--}}
                            <a class="btn btn-warning btn-sm" href="{{route('admin.payment-methods.index')}}" title="{{ trans('button.reset') }}"><i class="refresh ion ion-md-refresh"></i></a>
{{--                        </div>--}}
{{--                    </div>--}}
{{--                </form>--}}
            </div>
                ...
                <div class="card-body mt-0 pt-0">
                    <form id="update-pages" action="{{route('admin.payment-methods.actions')}}" method="post">
                        @csrf
                    </form>
                            ...
                            <tbody>
                                @forelse($items as $key => $item)
                                    <tr>
                                        ...
                                        <td>
                                            {{  substr(removeHTML( @$item->transNow?->description),0,30) }}
                                        </td>
                                        <td>
                                        <a href="{{asset($item->path() . $item->logo)}}" target="_blank">  <img src="{{asset($item->path() . $item->logo)}}" alt="" style="width: 50px"></a>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <div class="d-flex justify-content-center">
                                                @if($item->status == 'active')
                                                    <a href="{{ route('admin.payment-methods.update-status', $item->id) }}" class="btn btn-xs btn-success btn-sm m-1" title="{{ trans('button.active') }}"><i class="fa fa-check"></i></a>
                                                @else
                                                    <a href="{{ route('admin.payment-methods.update-status', $item->id) }}" class="btn btn-xs btn-outline-secondary btn-sm m-1" title="{{ trans('button.unactive') }}"><i class="fa fa-ban"></i></a>
                                                @endif

                                                <a href="{{ route('admin.payment-methods.show', $item->id) }}" class="btn btn-xs btn-outline-info btn-sm m-1" title="{{ trans('button.show') }}"><i class="fas fa-eye"></i></a>

                                                <a href="{{ route('admin.payment-methods.edit', $item->id) }}" class="btn btn-outline-primary btn-sm m-1" title="{{ trans('button.edit') }}"><i class="fas fa-pencil-alt"></i></a>
                                            ...
                                            <div class="modal fade" id="exampleModal{{ $item->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        <form  action="{{ route('admin.payment-methods.destroy' , $item->id) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
