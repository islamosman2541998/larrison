...
$hasActiveChild = $children->contains(function ($c) use ($locale, $currentPath) {
    $cUrl = LaravelLocalization::getLocalizedURL($locale, $c->type === 'dynamic' ? $c->dynamic_url : $c->url);
    ...
    return $cPath === $currentPath ||
        ($cPath !== '' && \Illuminate\Support\Str::startsWith($currentPath, $cPath));
});
...
$liClass = $isActive || $hasActiveChild ? 'active' : '';
    ...
    @if ($children->count())
        <li class="nav-item dropdown {{ $liClass }}">
            <a href="{{ $itemUrl }}" class="nav-link dropdown-toggle" id="navbarDropdown{{ $item->id }}"
    ...
    @else
        <li>
            <a class="nav-a {{ $isActive ? 'active' : '' }}" href="{{ $itemUrl }}">
